...
module VisNode=
    ...
    let getSocketClass  thisType (selectedType:Type option)  isInput=
        let io=
            if isInput then "input" else "output"
        let state=
            selectedType
            |>Option.map(fun x-> if(x= thisType)then "matching" else "disabled")
            |>Option.defaultValue ""
    ...
    let renderSockets names sockets selectedType (idPrefix:string) isInput=
        Html.ul[
            prop.className "socketList"
            prop.children ((names,sockets)||>List.mapi2(fun i name socketType->
                let render= renderPoint name (idPrefix+i.ToString()) socketType
                let className= getSocketClass socketType selectedType isInput
                render className
            ))
        ]
    ...
    let renderNode (visNode:VisNode) selectedType =
        Html.div[
            prop.className "node"
            prop.style[style.position.absolute; style.left (visNode.Postition|>fst); style.bottom (visNode.Postition|>snd) ]
            prop.children[
                Html.h1 visNode.Name
...
module Connection=
    ...
    let renderPoint thisType classExtra=
        ...
        //TODO
        Html.div[
            prop.className ("circle"+classExtra)
        ]
